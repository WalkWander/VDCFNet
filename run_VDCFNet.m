...
for frame = 2:num_frame
    [state, region] = VDCFNet_update(state, im{frame}, frame);
    result(frame,:) = region;
end
time = toc;
res.type = 'rect';
...
state.net_name = './VDCFNet-2017-7-20';
...
function [state, location] = VDCFNet_update(state, I, frame,varargin)
...
response = squeeze(real(ifft2(bsxfun(@times, state.model_alphaf, kzf))));
    ...
    if vert_delta > size(responseUP,1) / 2
        vert_delta = vert_delta - size(responseUP,1);
    end
    if horiz_delta > size(responseUP,2) / 2  
